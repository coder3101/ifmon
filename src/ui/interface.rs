...
pub fn render_interface_info(frame: &mut Frame, area: Rect, interface: &Interface, ip_scroll_offset: usize) {
    ...
    if interface.ipv4.is_empty() {
        ...
        for (i, ip) in interface.ipv4.iter().skip(ip_scroll_offset).take(2).enumerate() {
            rows.push(Row::new(vec![
                Span::styled(
                    if i == 0 { "IPv4" } else { "" },
    ...
    if interface.ipv6.is_empty() {
        ...
        for (i, ip) in interface.ipv6.iter().skip(ip_scroll_offset).take(1).enumerate() {
            rows.push(Row::new(vec![
                Span::styled(
                    if i == 0 { "IPv6" } else { "" },
                    Style::default().fg(Color::Yellow)
